...
# ===== スクール（U10/U12）: /school/schedule-data.js =====
python3 - "$SCRIPT_DIR/school" <<'EOF2'
    ...
    for dateKey, items in data.get("dayCalendarData", {}).items():
        for item in items:
            ...
            if ("U12" in t or "U10" in t) and "スクール" in t:
                d = data.get("detailData", {}).get(str(item["seq"]), {})
                time_str = d["startTime"] + "〜" + d["endTime"] if d.get("startTime") and d.get("endTime") else ""
                cls = "u12" if "U12" in t else "u10"
                events.setdefault(dateKey, []).append({"cls": cls, "label": cls.upper(), "time": time_str})

json_path = os.path.join(out_dir, "schedule.json")
if os.path.exists(json_path):
    try:
        with open(json_path, encoding="utf-8") as f:
            if json.load(f) == events:
                print(f"  スクール 変更なし: {len(events)}日分")
                sys.exit(0)
    except Exception:
        pass

lines = [f"// 自動更新: {datetime.now().strftime('%Y-%m-%d %H:%M')}", "const SCHEDULE_DATA = {"]
for k in sorted(events):
    lines.append(f'  "{k}": {json.dumps(events[k], ensure_ascii=False)},')
lines.append("};")
with open(os.path.join(out_dir, "schedule-data.js"), "w", encoding="utf-8") as f:
    f.write("\n".join(lines) + "\n")
with open(json_path, "w", encoding="utf-8") as f:
    json.dump(events, f, ensure_ascii=False)
print(f"  スクール 完了: {len(events)}日分")
EOF2
